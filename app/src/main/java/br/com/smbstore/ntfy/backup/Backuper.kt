package br.com.smbstore.ntfy.backup
...
import br.com.smbstore.ntfy.firebase.FirebaseMessenger
import br.com.smbstore.ntfy.msg.NotificationService
...
class Backuper(val context: Context) {
    ...
    private suspend fun applySubscriptions(subscriptions: List<Subscription>?) {
        ...
        subscriptions.forEach { s ->
            try {
                // Add to database
                val subscription = br.com.smbstore.ntfy.db.Subscription(
                    id = s.id,
                    baseUrl = s.baseUrl,
                    topic = s.topic,
                    instant = s.instant,
        ...
        notifications.forEach { n ->
            try {
                val actions = if (n.actions != null) {
                    n.actions.map { a ->
                        br.com.smbstore.ntfy.db.Action(
                            id = a.id,
                            action = a.action,
                            label = a.label,
                            clear = a.clear,
                ...
                val attachment = if (n.attachment != null) {
                    br.com.smbstore.ntfy.db.Attachment(
                ...
                repository.addNotification(br.com.smbstore.ntfy.db.Notification(
                    id = n.id,
    ...
    private suspend fun applyUsers(users: List<User>?) {
        ...
        users.forEach { u ->
            try {
                repository.addUser(br.com.smbstore.ntfy.db.User(
                    baseUrl = u.baseUrl,
                    username = u.username,
                    password = u.password
                ))
...
data class Attachment(
    ...
    val url: String, // URL (mandatory, see br.com.smbstore.ntfy server)
