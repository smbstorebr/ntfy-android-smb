package br.com.smbstore.ntfy.ui
    ...
    class SettingsFragment : PreferenceFragmentCompat() {
        ...
        override fun onCreatePreferences(savedInstanceState: Bundle?, rootKey: String?) {
            ...
            backup?.onPreferenceChangeListener = Preference.OnPreferenceChangeListener { _, v ->
                ...
                val suggestedFilename = when (backupSelection) {
                    BACKUP_EVERYTHING_NO_USERS -> "br.com.smbstore.ntfy-backup-no-users-$timestamp.json"
                    BACKUP_SETTINGS_ONLY -> "br.com.smbstore.ntfy-settings-$timestamp.json"
                    else -> "br.com.smbstore.ntfy-backup-$timestamp.json"
                }
            ...
            versionPref?.onPreferenceClickListener = OnPreferenceClickListener {
                ...
                val clip = ClipData.newPlainText("br.com.smbstore.ntfy version", version)
        ...
        private fun copyLogsToClipboard(scrub: Boolean) {
            lifecycleScope.launch(Dispatchers.IO) {
                ...
                requireActivity().runOnUiThread {
                    val clipboard = context.getSystemService(Context.CLIPBOARD_SERVICE) as ClipboardManager
                    val clip = ClipData.newPlainText("br.com.smbstore.ntfy logs", log)
                    clipboard.setPrimaryClip(clip)
                    if (scrub) {
